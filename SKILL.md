...
| "The rule says ≤5 lines / no try/catch, but the *intent* is satisfied even if the letter isn't — this is just HTTP shape, not domain logic." | "Intent satisfied" is the phrase that ships the un-redacted payload to the batch job. View selection driven by a query param is a policy decision (who sees PII), not HTTP shape — encode the policy as a service-layer enum and let the resource translate the wire string. The ≤5-line / no-`try/catch` rule is the *check*, not the principle; the principle is "the resource is one of N callers" — and "intent" reasoning concedes that any caller bypassing the resource bypasses the policy. |
